...
def instrucao_logicoaritmetica(tokens_da_linha, registradores_widget, console_widget):
    ...
    try:
        ...
        if instrucao == "div":
            try:
                registradores_widget.registradores[registrador_destino] = int(registradores_widget.registradores[registrador_operando1] / registradores_widget.registradores[registrador_operando2])
            except:
...
def instrucao_desvio(tokens_da_linha, registradores_widget, linha_atual_index):
    ...
    if tipo == "j":
        # Lógica para instrução de salto incondicional
        return registradores_widget.endereco_label[destino_label]
    
    elif tipo == "jal":
        registradores_widget.registradores['ra'] = linha_atual_index + 1
        return registradores_widget.endereco_label[destino_label].get('linha', None)
